...
def weighted_TR(out, trg, y, weight, den, beta=1., epsilon=1e-5):
    return beta * ((y.squeeze() - trg.squeeze() * weight / (den+epsilon) - out[1].squeeze()) ** 2).mean()
...
if __name__ == "__main__":
    ...
    parser.add_argument('--n_epochs', type=int, default=800,  # Adjusted to 800
        ...
        for _ in range(num_iterations):
            ...
            for model_name in models:
                if model_name == 'Vcnet' or model_name == 'Vcnet_TR':
                    cfg_density = [(498, 200, 1, 'relu'), (200, 50, 1, 'relu')]
                    num_grid = 10
                ...
                if model_name == 'Vcnet_TR' or model_name == 'weightednet_TR':
                    init_lr = 0.0001  # Adjusted learning rate
                    alpha = 0.5  # Adjusted alpha
                    tr_init_lr = 0.0001  # Adjusted TR learning rate
                    beta = 0.5
                elif model_name == 'Drnet_tr':
                    ...
                    for inputs, y in train_loader:
                        ...
                        if isTargetReg:
                            ...
                            if model_name == 'weightednet_TR':
                                den = torch.tensor(gaussian_kde(t)(
                                    t), dtype=torch.float32, device=trg.device)
                                loss = criterion_weighted(
                                    out, y, weights) + weighted_TR(out, trg, y, den, weights)
                            else:
                                loss = criterion(

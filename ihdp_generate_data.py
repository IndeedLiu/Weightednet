...
if __name__ == "__main__":
    ...
    parser.add_argument('--data_path', type=str,
                        default='dataset/ihdp/ihdp.csv', help='data path')
    parser.add_argument('--save_dir', type=str,
                        default='dataset/ihdp', help='dir to save generated data')
    ...
    tem = torch.tanh(
        (torch.sum(ihdp[:, cate_idx2], dim=1)/10. - cate_mean2) * alpha)
    ...
    def x_t(x):
        ...
        x4 = x[4]
        x5 = x[5]
        t = x1/(1. + x2) + max(x3, x4, x5)/(0.2 + min(x3, x4, x5)) + \
            torch.tanh((torch.sum(x[cate_idx2])/10. - cate_mean2) * alpha) - 2.
